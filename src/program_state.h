...
public:
	program_state();
	program_state(const std::vector<point_cloud_types::Dir>& translations_, std::vector<point_cloud_types::Qat> rotations_, constructed_set picked, constructed_set previous_picked, std::vector<constructed_set> group_informations_, std::vector<cgv::media::color<float, cgv::media::RGB, cgv::media::OPACITY>> component_colors_, cgv::media::color<float, cgv::media::RGB, cgv::media::OPACITY> old_color_);
